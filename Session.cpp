...
//Function for adding user to score
void Session::addToScore(int s)
{
  ...
  if (u) {
    u.modify()->amount += s;
    ++u.modify()->isOrdered;
    u.modify()->lastOrder = WDateTime::currentDateTime();
  }
...
int Session::findRanking()
{
  ...
  if (u)
    ranking = session_.query<int>("select distinct count(amount) from user")
      .where("score > ?").bind(u->amount);
